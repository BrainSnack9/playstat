...
export default async function BasketballTeamsPage({ params }: Props) {
  const { locale } = await params
  ...
  // Group by conference (Eastern/Western)
  const easternTeams = teams.filter((t) => t.name.includes('Eastern') || t.externalId?.includes('east'))
  const westernTeams = teams.filter((t) => t.name.includes('Western') || t.externalId?.includes('west'))
  const otherTeams = teams.filter(
    (t) => !easternTeams.includes(t) && !westernTeams.includes(t)
  )
